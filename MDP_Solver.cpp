...
class PROBLEM {
    public:
        ...
        void GRASP(int D[C][C], int m, int time_max, string name){
            ...
            while (chrono::duration_cast<chrono::seconds>(chrono::steady_clock::now() - begin).count() < time_max) {
                ...
                times.push_back(chrono::duration_cast<chrono::milliseconds>(t1 - t0).count());
                //CUIDADO alpha fijo.
                if (Of > Of_best) {
                    M_best = M;
                    Of_best = Of;
        ...
        void TABU2(int D[C][C], int m, int time_max, string name) {
            Construction_Random(D, m);
            Tabu_Search2(D, m, time_max);
            Write_instance(M_best, Of_best, name);
        }
    ...
    private:
        ...
        void Tabu_Search2(int D[C][C], int m, int time_max) {
            ...
            while (chrono::duration_cast<chrono::seconds>(chrono::steady_clock::now() - begin).count() < time_max) {
                ...
                for (int i = 0; i < M.size(); i++) {
                    ...
                    TABU[M[i]] = iteration;
                    if (TABU[M[i]] == 2 * tabu_tenure) {
                        iteration = tabu_tenure + 1;
                        std::fill(TABU, TABU + C, 0);
                    }
                    cont.update(max, i, D, Of, M, val);
                    iteration++;
                    if (Of_best < Of) {
...
int main() {
    ...
    if (answer.size() == 4) {
        ...
        if (opt == 0) {
            ...
            instance.TABU2(D, m, timer, "result");
        }
        ...
        for (int j = 0; j < answer.size()-3; j++) {
            ...
            if (opt == 0) {
                ...
                instance.TABU2(D, m, timer, "tabu_" + to_string(timer) + "s_" + to_string(j));
            }
        }
...
vector<string> Menu() {
    ...
    */
    ...
    do {
        ...
        do {
            cout << endl << "Please type the algorithm to use" << endl << "Option 0 - GRASP"<<endl<< "Option 1 - Tabu"<<endl<<"Option 2 - Modified Tabu"<<endl << endl;
            cin >> opt;
        } while (stoi(opt) != 0 && stoi(opt)!=1 && stoi(opt) != 2);
